...
class CardActor(
    ...
    /** Приглушить — например, недоступную для розыгрыша карту. */
    ...
    override fun draw(batch: Batch, parentAlpha: Float) {
        ...
        val alpha = color.a * parentAlpha * if (dimmed) 0.55f else 1f
        ...
        if (faceUp) {
            ...
            if (letter != null) drawLetter(batch, letter, alpha)
            if (stackCount > 1) drawStackCount(batch, alpha)
        }
        batch.setColor(Color.WHITE)
    }

    private fun drawLetter(batch: Batch, letter: Letter, alpha: Float) {
        val font = assets.cardLetterFont
        val scale = width * LETTER_SCALE / font.capHeight
        font.data.setScale(scale)
        val accent = Palette.school(letter)
        font.color = Color(accent.r, accent.g, accent.b, alpha)
    ...
    private fun drawStackCount(batch: Batch, alpha: Float) {
        val font = assets.bodyBoldFont
        val scale = width * COUNT_SCALE / font.capHeight
        font.data.setScale(scale)
        font.color = Color(Palette.GOLD_LIGHT.r, Palette.GOLD_LIGHT.g, Palette.GOLD_LIGHT.b, alpha)
