...
class GameScreen(
    ...
    private fun advance(delta: Float) {
        // Окно выбора живёт ровно столько, сколько партия ждёт от нас ответа.
        // Раньше его снимало только касание карты: если выбор дошёл до сервера
        // (вымпел уже висит, ход перешёл сопернику), окно оставалось поверх
        // чужого хода, и убрать его можно было только выбрав карту ещё раз.
        // Проверяем до простоя режиссёра: снимать его надо сразу, а не после
        // того, как отыграют анимации собственного выбора.
        if (choiceDialog != null && !(state.phase == Phase.AWAITING_CHOICE && state.actingSide == Side.YOU)) {
            dismissChoiceDialog()
        }
