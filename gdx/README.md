...
./gradlew lwjgl3:run -Pfirst.net=surrender          # партия со сдачей: проверка исхода
...
| Что | Как получить |
|---|---|
| Windows и macOS, готовые приложения | Actions → «Десктопные сборки» → Run workflow |
| Кроссплатформенный jar | `./gradlew lwjgl3:jar`, нужна установленная Java 17 |
| Веб | `./gradlew :teavm:gdx_teavm_web_js_build` |
| Android APK | `./gradlew android:assembleDebug` |

Десктопные сборки собираются вручную и только на раннерах Windows и macOS:
кросс-компиляции у `jpackage` нет, а минуты этих раннеров в бесплатном тарифе
считаются с множителем 2 и 10. В архиве лежит приложение со встроенной Java —
распаковать и запустить, ставить ничего не нужно. Если указать в запуске тег
релиза, архивы приклеятся к нему черновиком.

Сборки не подписаны: macOS попросит открыть приложение через правый клик →
«Открыть», Windows покажет предупреждение SmartScreen. Подпись требует платных
сертификатов Apple и Microsoft.
