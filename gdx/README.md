...
### Отладочные ключи веба

Системных свойств в браузере нет, поэтому ключи приходят строкой запроса:

```
?net=host | ?net=guest      # окно ведёт партию само: хозяин или гость
?profile=b                  # отдельные настройки: два окна как разные игроки
?boot=online                # сразу список комнат
?keyboard=1                 # клавиатура телефона в настольном браузере
```

Последний включает мост к полю ввода страницы. Обычно он поднимается только на
устройствах с касанием — на телефоне без него не набрать ни ника, ни кода
комнаты, — а с этим ключом его видно и на десктопе.
